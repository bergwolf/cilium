...
func (r *rule) canReach(ctx *SearchContext) api.Decision {
	...
	for _, r := range r.Ingress {
		for _, sel := range r.FromRequires {
			policyTraceVerbose(ctx, "Rule %s: requires labels %+v\n", r, sel)

			// TODO: get rid of this cast
			if !ctx.From.Contains(labels.LabelArray(sel)) {
				policyTrace(ctx, "  Required labels not found\n")
				return api.Denied
			}
		}
	}
	...
	for _, r := range r.Ingress {
		for _, sel := range r.FromEndpoints {
			policyTraceVerbose(ctx, "Rule %s: allows labels %+v\n", r, sel)

			// TODO: get rid of this cast
			if ctx.From.Contains(labels.LabelArray(sel)) {
				policyTrace(ctx, "  Found all required labels\n")
				return api.Allowed
			}
	...
	policyTraceVerbose(ctx, "rule %s: no FromEndpoints\n", r)
